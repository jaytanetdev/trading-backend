...
function rsi(values, period = 14) {
    ...
    for (let i = 1; i < values.length; i++) {
        ...
        if (i <= period) {
            ...
            const prevAvgGain = ((gain / period) * (period - 1)) / period +
                (diff > 0 ? diff / period : 0);
            const prevAvgLoss = ((loss / period) * (period - 1)) / period +
                (diff < 0 ? -diff / period : 0);
...
function detectSupportResistance(candles, lookback = 5, clusterTolerancePct = 0.015) {
    const highs = [];
    const lows = [];
...
function analyzeStock(candles) {
    ...
    const firstHalfAvg = recent.slice(0, 10).reduce((a, c) => a + c.close, 0) / 10;
    const secondHalfAvg = recent.slice(10).reduce((a, c) => a + c.close, 0) / 10;
    weight += 0.5;
    ...
    const nearestSupport = sr.support[sr.support.length - 1] ?? last.close - atr14 * 2;
